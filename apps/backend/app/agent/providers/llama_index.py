...
# Import ChatMessage and MessageRole for the chat logic
...
class LlamaIndexProvider(Provider):
    def __init__(self,
        ...
        # --- MODIFICATION 1: Set is_chat_model based on provider ---
        # Check if we are using Groq and set it as a chat model
        if 'groq' in self._modname.lower():
             kwargs_for_provider['is_chat_model'] = True
        else:
             kwargs_for_provider['is_chat_model'] = False
        # --- END MODIFICATION 1 ---
    ...
    def _generate_sync(self, prompt: str, **options) -> str:
        ...
        """
        try:
            # --- MODIFICATION 2: Use .chat() for chat models ---
            # Use .chat() for chat models and .complete() for others
            if self._client.is_chat_model:
